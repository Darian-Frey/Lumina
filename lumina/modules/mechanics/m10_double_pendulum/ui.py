...
class DoublePendulumWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        pl = QVBoxLayout(phys)
        row, self._spin_m1 = _spin_row("Mass m\u2081 (kg):", 0.1, 10.0, DEFAULT_M1)
        pl.addLayout(row)
        row, self._spin_m2 = _spin_row("Mass m\u2082 (kg):", 0.1, 10.0, DEFAULT_M2)
        pl.addLayout(row)
        row, self._spin_L1 = _spin_row("Length L\u2081 (m):", 0.1, 5.0, DEFAULT_L1)
        pl.addLayout(row)
        row, self._spin_L2 = _spin_row("Length L\u2082 (m):", 0.1, 5.0, DEFAULT_L2)
        pl.addLayout(row)
        row, self._spin_g = _spin_row("Gravity g:", 0.1, 20.0, DEFAULT_G)
        pl.addLayout(row)
        ctrl.addWidget(phys)
        ...
        il = QVBoxLayout(ic)
        row, self._spin_th1 = _spin_row("Angle \u03b8\u2081 (rad):", -np.pi, np.pi, DEFAULT_THETA1)
        il.addLayout(row)
        row, self._spin_th2 = _spin_row("Angle \u03b8\u2082 (rad):", -np.pi, np.pi, DEFAULT_THETA2)
        il.addLayout(row)
        row, self._spin_w1 = _spin_row("\u03c9\u2081 (rad/s):", -10.0, 10.0, DEFAULT_OMEGA1)
        il.addLayout(row)
        row, self._spin_w2 = _spin_row("\u03c9\u2082 (rad/s):", -10.0, 10.0, DEFAULT_OMEGA2)
        il.addLayout(row)
        ctrl.addWidget(ic)
        ...
        al.addLayout(row)
        row, self._spin_trail = _spin_row("Trail length:", 50, 3000, 600, 0)
        al.addLayout(row)
